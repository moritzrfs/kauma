...
def handle_pkcs7_padding(assignment):
    ...
    # plain base64 encode and decode utf8
...
def decrypt_cbc(xor_result: bytearray, iv: bytearray):
    ...
    # get last byte of plaintext
    last_byte = plaintext[-1]
    # check if last x bytes of plaintext are equal to last_byte
    if plaintext[-last_byte:] == bytes([last_byte] * last_byte):
        # remove last x bytes from plaintext
